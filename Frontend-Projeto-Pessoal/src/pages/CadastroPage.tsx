...
import { Eye, EyeOff, Zap, Mail, User, Lock, Shield, Briefcase, GraduationCap, ArrowLeft, Check } from "lucide-react";
import { motion } from "framer-motion";
import { apiCadastro, type ErroResposta } from "../api/client";

const ROLES = [
  { value: "GERENTE",    label: "Gerente",    icon: Shield,        color: "#f59e0b", bg: "rgba(245,158,11,0.08)"   },
  { value: "FUNCIONARIO", label: "Funcionário", icon: Briefcase,    color: "#00f0ff", bg: "rgba(0,240,255,0.08)"   },
  { value: "ESTAGIARIO", label: "Estagiário", icon: GraduationCap, color: "#a78bfa", bg: "rgba(167,139,250,0.08)" },
];
...
export function CadastroPage() {
  ...
  const [roles, setRoles]     = useState<string[]>(["FUNCIONARIO"]);
  const [errors, setErrors]   = useState<Record<string, string>>({});
  const [globalErr, setGlobalErr] = useState("");
  const [submitting, setSubmitting] = useState(false);

  // Seleção exclusiva — cada usuário tem um único perfil
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await apiCadastro({ nome, login: loginV, email, senha, roles });
    if (res.status === 201) {
              ...
              {ROLES.map(({ value, label, icon: Icon, color, bg }) => {
                ...
                return (
                  <button
                    ...
                    className="relative flex flex-col items-center justify-center gap-1.5 py-3 rounded-xl text-sm font-medium transition-all"
                    style={{
                  ...
                  >
                    ...
                    {active && <Check size={11} className="absolute top-1.5 right-1.5" />}
                  </button>
                );
              })}
            </div>
