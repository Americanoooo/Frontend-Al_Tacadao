...
export async function apiListUsuarios(): Promise<MeResponse[]> {
...
export async function apiListEstoque(): Promise<Produto[]> {
  const res = await fetch("/api/estoque?tamanha-pagina=1000", { headers: h, credentials: "include" });
...
export async function apiDeleteProduto(id: string): Promise<Response> {
  return fetch(`/api/estoque/${id}`, { method: "DELETE", headers: h, credentials: "include" });
}
